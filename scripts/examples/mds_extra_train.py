...
def build_model(params, w_emb):
    ...
    # debug
    embs_l_sub = semi_mds.embed_layer.get_output_for(x_l_sub)
    cost_l = semi_mds.get_cost_L([x_l_sub, embs_l_sub, m_l_sub, d_l, y_l], kl_w)

    inputs_l = [x_l_all, m_l_all, x_l_sub, m_l_sub, d_l, y_l]
    inputs_u = [x_u_all, m_u_all, x_u_sub, m_u_sub, d_u]
    cost, train_acc = semi_mds.get_cost_together(inputs_l, inputs_u, kl_w)
    test_acc = semi_mds.get_cost_test([x_l_all, m_l_all, d_l, y_l])

    network_params = semi_mds.get_params(only_trainable = True)
    ...
    f_debug = theano.function([x_l_sub, m_l_sub, d_l, y_l, kl_w], cost_l)
    f_train = theano.function(inputs_l + inputs_u + [kl_w], [cost, train_acc], updates = params_update)
    f_test = theano.function([x_l_all, m_l_all, d_l, y_l], test_acc)

    return semi_mds, f_debug, f_train, f_test
...
def train(params):
    ...
    semi_mds, f_debug, f_train, f_test = build_model(params, w_emb)
