# Note that this implementation omits the original alpha in paper.
...
class ScLSTMLayer(MergeLayer):
    ...
    """
    def __init__(self, incoming, num_units, da_init,
                 ingate=Gate(),
                 forgetgate=Gate(),
        ...
        if isinstance(da_init, Layer):
            incomings.append(da_init)
            self.da_init_incoming_index = len(incomings)-1

        # Initialize parent layer
        super(ScLSTMLayer, self).__init__(incomings, **kwargs)
        ...
        num_da_units = self.input_shapes[self.da_init_incoming_index][1]

        # add _num_units param to fit various gate
        def add_gate_params(gate, gate_name, _num_units):
            """ Convenience function for adding layer parameters from a Gate
            instance. """
            return (self.add_param(gate.W_in, (num_inputs, _num_units),
                                   name="W_in_to_{}".format(gate_name)),
                    self.add_param(gate.W_hid, (num_units, _num_units),
                                   name="W_hid_to_{}".format(gate_name)),
                    self.add_param(gate.b, (_num_units,),
        ...
        (self.W_in_to_ingate, self.W_hid_to_ingate, self.b_ingate,
         self.nonlinearity_ingate) = add_gate_params(ingate, 'ingate', num_units)

        (self.W_in_to_forgetgate, self.W_hid_to_forgetgate, self.b_forgetgate,
         self.nonlinearity_forgetgate) = add_gate_params(forgetgate,
                                                         'forgetgate', num_units)

        (self.W_in_to_cell, self.W_hid_to_cell, self.b_cell,
         self.nonlinearity_cell) = add_gate_params(cell, 'cell', num_units)

        (self.W_in_to_outgate, self.W_hid_to_outgate, self.b_outgate,
         self.nonlinearity_outgate) = add_gate_params(outgate, 'outgate', num_units)

        (self.W_in_to_readgate, self.W_hid_to_readgate, self.b_readgate,
         self.nonlinearity_readgate)  = add_gate_params(readgate, 'readgate', num_da_units)

        self.W_da_to_cell = self.add_param(init.Normal(0.1), (num_da_units, num_units),
                                            name='W_da_to_cell')
        ...
        if isinstance(da_init, T.TensorVariable):
            if da_init.ndim != 2:
                raise ValueError(
                    "When hid_init is provided as a TensorVariable, it should "
                    "have 2 dimensions and have shape (num_batch, num_units)")
            self.da_init = da_init
        elif isinstance(da_init, Layer):
            self.da_init = da_init
        else:
            self.da_init = self.add_param(
                da_init, (1, self.num_units), name="da_init",
                trainable=learn_init, regularizable=False)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        if self.da_init_incoming_index > 0:
            da_init = inputs[self.da_init_incoming_index]

        print mask, hid_init, cell_init, da_init
        ...
        def step(input_n, cell_previous, hid_previous, da_previous, *args):
            ...
            da = readgate * da_previous # for this exp, da does not change.
        ...
        def step_masked(input_n, mask_n, cell_previous, hid_previous, da_previous, *args):
            cell, hid, da = step(input_n, cell_previous, hid_previous, da_previous, *args)
            ...
            da = da*mask_n + da_previous*not_mask
        ...
        if isinstance(self.da_init, Layer):
            pass
        elif isinstance(self.da_init, T.TensorVariable):
            da_init = self.da_init
        else:
            da_init = T.dot(ones, self.da_init)

        # The hidden-to-hidden weight matrix is always used in step
        non_seqs = [W_hid_stacked, self.W_da_to_cell]
        ...
        if self.unroll_scan:
            ...
            cell_out, hid_out, da_out = unroll_scan(
                fn=step_fun,
                sequences=sequences,
                outputs_info=[cell_init, hid_init, da_init],
            ...
            cell_out, hid_out, da_out = theano.scan(
                fn=step_fun,
                sequences=sequences,
                outputs_info=[cell_init, hid_init, da_init],
                go_backwards=self.backwards,
